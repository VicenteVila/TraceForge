...
Run with a local instance (see docker-compose.yml):
    TRACEFORGE_TEST_CH_URL="http://localhost:8123/default" \
        pytest tests/test_integration_clickhouse.py
"""
...
pytestmark = pytest.mark.skipif(
    not os.environ.get("TRACEFORGE_TEST_CH_URL"),
    reason="set TRACEFORGE_TEST_CH_URL to run ClickHouse integration tests",
)


@pytest.fixture
def collector():
    c = ClickHouseCollector(os.environ["TRACEFORGE_TEST_CH_URL"])
    c.clear()
    yield c
    c.clear()
    c.close()
